...
select product,
Estimate_sale_price-Purchase_cost
from bank_inventory_pricing
where  Estimated_sale_price- Purchase_cost > 1
group by product;
...
select branch, sum(estimated_profit),
revenue - cost as real_profit
from Bank_branch_PL
group by branch;
...
select product, estimated_profit
from Bank_branch_PL

group by product
having sum(revenue - cost) between sum(0.1*estimated_profit) and sum(estimated_profit);
...
from Bank_branch_PL
where (revenue-(cost-(cost*0.3))) > estimated_profit;
